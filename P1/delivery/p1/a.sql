...
INSERT INTO invalid_blocks_temp
-- Get blocks containing transactions where inputs have no corresponding valid output
SELECT DISTINCT block_id
...
-- Get the input_id of the where the output was first spent
    ...
    FROM double_spent_outputs
    JOIN non_coinbase_inputs
    USING (output_id)
    GROUP BY output_id
)
INSERT INTO invalid_blocks_temp
...
SELECT block_id
    ...
    JOIN non_coinbase_inputs
    USING (output_id)
    WHERE input_id NOT IN (SELECT input_id FROM first_spent);
...
FROM transactions JOIN (
    SELECT non_coinbase_inputs.tx_id AS tx_id, SUM(value) AS sum
...
SELECT DISTINCT block_id
FROM coinbase_values
JOIN input_values USING (block_id)
JOIN output_values USING (block_id)
WHERE coinbase_values.coinbase_value > (5000000000 + (input_values.total_inputs - output_values.total_outputs));

-- [TESTING] Clear invalid_blocks table
...
-- [TESTING] Get invalid block count and sha256 hash
